...
def train(dataset):
    ...
    logstr = f"Epoch {epoch:02d} - Train: " + ", ".join(
        f"{key}: {val:.3f}" for key, val in metrics_vals.items()
    )
...
def val(dataset):
    ...
    metrics_vals = metrics.evaluate()
    logstr = f"Epoch {epoch:02d} - Val: " + ", ".join(
        f"{key}: {val:.3f}" for key, val in metrics_vals.items()
    )
...
if __name__ == "__main__":
    ...
    cli_args = docopt(__doc__, version="Usecase 3 Training Script 1.0")
    ...
    # train = hed_data/train
    # val   = hed_data/val
    # ========================================================
    trn_dataset = Augment(get_dataset('train'))
